...
// Select scatter, append SVG area to it, and set its width and height
...
var chartData = null;  // Store the chart data
...
function scale(chartData, chosenAxis, xy) {
    var axisRange = (xy === "x") ? [0, chartWidth]:[chartHeight, 0]
    
    // create scales for chosen axis
    var linearScale = d3.scaleLinear()
      .domain([d3.min(chartData, d => d[chosenAxis]) * 0.9,
...
function updateChart() {
    ...
    if (value !== chosenAxis) {
        ...
        linearScale = scale(chartData, chosenAxis, xy);
...
function init() {
    ...
    var xLinearScale = scale(chartData, chosenXAxis, "x");
    var yLinearScale = scale(chartData, chosenYAxis, "y");
